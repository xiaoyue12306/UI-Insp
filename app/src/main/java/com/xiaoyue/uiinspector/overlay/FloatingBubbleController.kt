...
class FloatingBubbleController(context: Context, private val host: OverlayController, private val prefs: InspectorPreferences,
    ...
    val view = ImageButton(context).apply {
        ...
        background = GradientDrawable().apply { shape=GradientDrawable.OVAL; setColor(Color.WHITE); setStroke(host.dp(1),0xffdddddd.toInt()) }
        clipToOutline=true
    ...
    fun selecting(value: Boolean) {
        view.setImageResource(if(value) android.R.drawable.ic_menu_close_clear_cancel else R.drawable.ic_magnifier)
        if(value) view.setColorFilter(0xff284b63.toInt()) else view.clearColorFilter()
